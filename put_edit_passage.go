...
func PutEditPassage(router *mux.Router, conn *pgxpool.Pool) {
	router.HandleFunc("/passages/{Id}", func(w http.ResponseWriter, r *http.Request) {
		...
		_, err = conn.Exec(context.Background(), query, id, reference.Book, reference.StartChapter, reference.StartVerse, reference.EndChapter, reference.EndVerse, r.FormValue("text"), r.FormValue("level"), reviewAt)
		if err != nil {
			println(err.Error())
